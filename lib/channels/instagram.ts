...
async function findUserByIgAccountId(accountId: string) {
...
export async function handleInstagramWebhook(body: unknown) {
  ...
  for (const entry of payload.entry ?? []) {
    ...
    const userId = await findUserByIgAccountId(accountId);
    if (!userId) continue;
    ...
    for (const event of entry.messaging ?? []) {
      ...
      if (token && senderId) {
        ...
        logger.info("Instagram mock reply", { orderId: order?.id });
      }
    }
  }
}
